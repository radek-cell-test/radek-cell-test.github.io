/* ==========================================================================
   LabCal — stabilisation & cycle-time checks (v1.551)
   --------------------------------------------------------------------------
...
(function (global) {
  ...
  function pair(fromIds, toIds, minGap, calcId, texts) {
    ...
    var problem = null;
    if (diff < 0) problem = { msg: texts.reversed, focusId: toIds[0] };
    else if (diff < minGap) problem = { msg: texts.short + ' (now ' + diff + ' min)', focusId: toIds[0] };
  ...
  function check() {
    ...
    var p = pair(['af_probes_in_h', 'af_probes_in_m'], ['af_cycle_start_h', 'af_cycle_start_m'], STAB_MIN, 'af_stabilisation', {
      reversed: 'AF: Cycle start cannot be earlier than Probes loaded at',
      short: 'AF: stabilisation must be at least 30 min (Probes loaded at → Cycle start)'
    });
    if (p) problems.push(p);
    p = pair(['af_cycle_start_h', 'af_cycle_start_m'], ['af_cycle_end_h', 'af_cycle_end_m'], cyc, 'af_cycletime', {
      reversed: 'AF: Cycle end cannot be earlier than Cycle start',
      short: 'AF: cycle time must be at least ' + cyc + ' min for ' + setLabel + ' (Cycle start → Cycle end)'
    });
    if (p) problems.push(p);
    ...
    if (alActive()) {
      ...
      p = pair(['af_cycle_end_h', 'af_cycle_end_m'], ['al_adjusted_h', 'al_adjusted_m'], 0, null, {
        reversed: 'AL: Adjusted at cannot be earlier than AF Cycle end',
        short: ''
      });
      if (p) problems.push(p);
      p = pair(['al_adjusted_h', 'al_adjusted_m'], ['al_cycle_start_h', 'al_cycle_start_m'], STAB_MIN, 'al_stabilisation', {
        reversed: 'AL: Cycle start cannot be earlier than Adjusted at',
        short: 'AL: stabilisation must be at least 30 min (Adjusted at → Cycle start)'
      });
      if (p) problems.push(p);
      p = pair(['al_cycle_start_h', 'al_cycle_start_m'], ['al_cycle_end_h', 'al_cycle_end_m'], cyc, 'al_cycletime', {
        reversed: 'AL: Cycle end cannot be earlier than Cycle start',
        short: 'AL: cycle time must be at least ' + cyc + ' min for ' + setLabel + ' (Cycle start → Cycle end)'
      });
      if (p) problems.push(p);
    } else {
