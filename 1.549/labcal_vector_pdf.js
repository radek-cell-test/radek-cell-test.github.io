...
(function (global) {
  ...
  function buildIBB() {
    ...
    if (ctlMode === 'none') {
      ...
      // Chart Air/Chart Load offsets get their own line directly under
      // Air/Load's, in the same box, rather than being written out only as
      // a Comments-page note (the old workaround, now retired — see
      // v1.536). No "Initial offsets"/"Final offsets" label is repeated on
      // this second line; the indentation under the line above already
      // groups it, and that saves the width for the two values themselves.
      function ctLineChart(top, offVal, greyed) {
        var mid = top + CT_ROW - 2.2;
        var x = MX + LAB_W + 2;
        [['Chart Air:', offVal[0]], ['Chart Load:', offVal[1]]].forEach(function (pair, i) {
          var bx = x + i * 32;
          e.t(pair[0], bx, mid, 6.8);
          if (greyed) e.chip(pair[1], bx + 15, mid - 3.1, 12);
          else e.t(pair[1] || '\u2014', bx + 16, mid, 7.8, 'bold');
        });
      }
      var initOff = [val('initialOffsetsCal1'), val('initialOffsetsCal2')];
      var initChartOff = [val('initialOffsetsCal3'), val('initialOffsetsCal4')];
      var finalOff = alNotNeededYet ? ['N/A', 'N/A'] : [val('finalOffsetsCal1'), val('finalOffsetsCal2')];
      var finalChartOff = alNotNeededYet ? ['N/A', 'N/A'] : [val('finalOffsetsCal3'), val('finalOffsetsCal4')];
      ctLine(ctTop, 'Initial offsets', initOff, 'Initial set point', val('initialSetpoint'),
             (txtOf('initialNearestPoint') || '').replace('\u2014', ''), false);
      ctLineChart(ctTop + CT_ROW, initChartOff, false);
      ctLine(ctTop + CT_ROW * 2, 'Final offsets', finalOff,
             'Final set point', alNotNeededYet ? '\u2013N/A\u2013' : val('finalSetpoint'),
             alNotNeededYet ? '' : (txtOf('finalNearestPoint') || '').replace('\u2014', ''), alNotNeededYet);
      ctLineChart(ctTop + CT_ROW * 3, finalChartOff, alNotNeededYet);
    }
    ...
    // ---------------- timing ----------------
    // Display cycle Max/Min/Average now print inside the measurement table
    // itself (see the "Display Max/Min (from product)" rows in table() above),
    // right alongside Air's and Load's, so this band only needs to carry the
    // AF/AL timings — Probes loaded at / Adjusted at, Cycle start and the
    // calculated Stabilisation Time — plus the AL Adjustment made note.
    var TW = [30, 30, 30, 26];
    ...
    var tcx = MX;
    ['Probes loaded at / Adjusted at', 'Cycle start', 'Stabilisation Time', 'AL Adjustment made'].forEach(function (lab, i) {
      e.t(lab, tcx + TW[i] / 2, y + thh - 1.4, 7.3, 'bold', INK, 'center');
    ...
    var trh = 5.4;
    [
      { label: 'AF — Probes loaded at', hId: 'af_probes_in_h', mId: 'af_probes_in_m',
        startH: 'af_cycle_start_h', startM: 'af_cycle_start_m', stabId: 'af_stabilisation', na: false },
      { label: 'AL — Adjusted at', hId: 'al_adjusted_h', mId: 'al_adjusted_m',
        startH: 'al_cycle_start_h', startM: 'al_cycle_start_m', stabId: 'al_stabilisation', na: alNotNeededYet }
    ].forEach(function (r, idx) {
      var top = y, grey = r.na;
      var tint = grey ? GREY_BG : GREEN_BG;
      var cxx = MX;
      for (var i = 0; i <= 2; i++) { e.box(cxx + 0.2, top + 0.2, TW[i] - 0.4, trh - 0.4, tint, null); cxx += TW[i]; }
      cxx = MX;
      if (grey) {
        e.chip('N/A', cxx + 4, top + 1.2, TW[0] - 8);
        e.chip('N/A', cxx + TW[0] + 4, top + 1.2, TW[1] - 8);
        e.chip('N/A', cxx + TW[0] + TW[1] + 4, top + 1.2, TW[2] - 8);
      } else {
        e.t(r.label, cxx + 1.5, top + 2, 5.6, 'normal', GREY_TXT);
        var c0 = cxx + TW[0] / 2;
        e.t(val(r.hId) || '—', c0 - 6, top + trh - 1.2, 8.5, 'bold', INK, 'center');
        e.t(':', c0, top + trh - 1.2, 8, 'normal', INK, 'center');
        e.t(val(r.mId) || '—', c0 + 6, top + trh - 1.2, 8.5, 'bold', INK, 'center');
        var c1 = cxx + TW[0] + TW[1] / 2;
        e.t(val(r.startH) || '—', c1 - 6, top + trh - 1.2, 8.5, 'bold', INK, 'center');
        e.t(':', c1, top + trh - 1.2, 8, 'normal', INK, 'center');
        e.t(val(r.startM) || '—', c1 + 6, top + trh - 1.2, 8.5, 'bold', INK, 'center');
        var c2 = cxx + TW[0] + TW[1] + TW[2] / 2;
        e.t(dash(txtOf(r.stabId) || val(r.stabId)), c2, top + trh - 1.8, 8.5, 'bold', INK, 'center');
      }
      var tx = MX + TW[0] + TW[1] + TW[2];
      if (idx === 1) {
        e.chip(val('al_adj_made') || (alNotNeededYet ? 'Adjustment not needed' : 'Adjustment made'),
               tx + 1, top + 1.2, TW[3] - 2);
      }
  ...
  function buildCloudTemp(opts) {
    ...
    function drawFields(y) {
      ...
      var date = val('date') || val('dateNative');
    ...
    function drawFooter(y) {
      ...
      [['Engineer Name', val('engineer'), txtOf('engSignature') || val('engineer'), val('engDate') || val('date'), true],
       // v1.549: no checker named = no checker date on the certificate
       ['Checked By (Name)', val('checkedBy'), txtOf('checkSignature'), val('checkedBy') ? val('checkDate') : '', false]
  ...
  global.LabCalVectorPdf = {
    ...
    blobSNMD: function () { return finish(buildSNMD('snmd')); },
    buildSMD: function () { return buildSNMD('smd'); },
    buildNSMD: function () { return buildSNMD('nsmd'); },
    buildIBB: buildIBB,
    buildJobSummary: buildJobSummary,
    blobJobSummary: function (job, progress) { return finish(buildJobSummary(job, progress)); },
    blobSMD: function () { return finish(buildSNMD('smd')); },
    blobNSMD: function () { return finish(buildSNMD('nsmd')); },
    blobIBB: function () { return finish(buildIBB()); },
    build19_24: build19_24,
    blob19_24: function () { return finish(build19_24()); },
    buildBarkey: buildBarkey,
    blobBarkey: function () { return finish(buildBarkey()); },
    blobCloudTemp: function (opts) { return finish(buildCloudTemp(opts)); }
  };
