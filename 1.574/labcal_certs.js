(window.LabCalBuild = window.LabCalBuild || {})['labcal_certs.js'] = 'v1.574';  // file version — see labcal_build.js
/* ---------------------------------------------------------------------
...
(function (global) {
  ...
  // be read within 2.5 s, generation continues.
  ...
  function confirmCertRefFree(certRef, unit) {
    var timedOut = false;
    var check = findCertRefUse(certRef, unit).then(function (clashes) {
      // too late — generation already went ahead; never pop a question up
      // in the middle of saving/sharing
      if (timedOut) return true;
      if (!clashes.length) return true;
      var c = clashes[clashes.length - 1];
      var what = [c.serial ? 'serial ' + c.serial : 'no serial', c.model, c.site, c.jobRef ? 'job ' + c.jobRef : '', c.day]
        .filter(Boolean).join(' · ');
      return global.confirm(
        'Certificate No. ' + String(certRef).trim() + ' has already been used for a DIFFERENT unit:\n\n' +
        what + (clashes.length > 1 ? '\n(+' + (clashes.length - 1) + ' more)' : '') +
    ...
    return Promise.race([check, timeout]).then(function (r) {
      if (r === 'timeout') { timedOut = true; console.warn('Certificate number check skipped — storage did not respond.'); return true; }
      return r;
    });
  }
