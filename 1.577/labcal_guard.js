(window.LabCalBuild = window.LabCalBuild || {})['labcal_guard.js'] = 'v1.577';  // file version — see labcal_build.js
/* ---------------------------------------------------------------------
...
(function (global) {
  ...
  // v1.569 (Radek): characters the certificate cannot print are NOT allowed.
  // The PDF fonts carry Latin-1 plus a few typographic marks (the same set
  // labcal_vector_pdf.js keeps); anything else — Welsh ŵ/ŷ, Polish ł/ś, emoji —
  // used to be dropped silently ("Ysbyty Tŷ Gwyn" printed "Ysbyty T Gwyn").
  // Now Generate stops and names the field and the character instead.
  // "…" (iPad smart punctuation) is printed as "..." so it is allowed.
  var PDF_OK = /[\s\x20-\xFF–—‘’“”•€…]/;
  function badChars(text) {
    var out = [];
    Array.from(String(text || '')).forEach(function (c) {
      if (!PDF_OK.test(c) && out.indexOf(c) === -1) out.push(c);
    });
    return out;
  }
  var FIELD_NAMES = { comments: 'Comments', certNo: 'Certificate No', rtMax: 'Room temp Max', rtMin: 'Room temp Min', al_adj_made: 'Adjustment made', initialSetpoint: 'Initial set point', finalSetpoint: 'Final set point' };
  function fieldLabel(el) {
    if (el.id && FIELD_NAMES[el.id]) return FIELD_NAMES[el.id];
    if (/^af_/.test(el.id || '')) return 'As Found readings';
    if (/^al_/.test(el.id || '')) return 'As Left readings';
    try {
      var box = el.closest('td, .field, label, div');
      var lab = box && box.querySelector('label');
      var t = lab ? lab.textContent : '';
      if (!t && box) {
        var clone = box.cloneNode(true);
        Array.prototype.forEach.call(clone.querySelectorAll('input, textarea, select, span.signature'), function (n) { n.remove(); });
        t = clone.textContent;
      }
      t = String(t || '').replace(/[*:]/g, ' ').replace(/\s+/g, ' ').trim();
      if (t && t.length <= 40) return t;
    } catch (e) {}
    return el.id || el.name || 'a field';
  }
  function unprintableFields(doc) {
    var found = [];
    if (!doc || !doc.querySelectorAll) return found;
    var els = doc.querySelectorAll('input, textarea');
    Array.prototype.forEach.call(els, function (el) {
      var type = String(el.type || 'text').toLowerCase();
      if (['text', 'search', 'textarea', ''].indexOf(type) === -1 && el.tagName !== 'TEXTAREA') return;
      var shown = el.offsetParent !== null || (el.getClientRects && el.getClientRects().length > 0);
      if (!shown) return;
      var bad = badChars(el.value);
      if (bad.length) found.push({ el: el, label: fieldLabel(el), chars: bad });
    });
    return found;
  }
  ...
  function beforeGenerate(ctx) {
    ...
    var bad = [];
    try { bad = unprintableFields(global.document); } catch (e) { console.warn('Character check skipped:', e); }
    if (bad.length) {
      global.alert('Special characters are not allowed on a certificate:\n\n' +
        bad.slice(0, 5).map(function (b) { return '\u2022 ' + b.label + ': ' + b.chars.join(' '); }).join('\n') +
        (bad.length > 5 ? '\n\u2022 \u2026 and ' + (bad.length - 5) + ' more' : '') +
        '\n\nReplace them with plain letters (e.g. \u0177 \u2192 y, \u0141 \u2192 L).\n\nNo certificate was made.');
      try { bad[0].el.focus(); } catch (e) {}
      return Promise.resolve({ ok: false, rev: 1, uid: '' });
    }
    ...
    // v1.576: if storage answers late, generation goes ahead as rev 1 (as
    // before) but the check finishes in the background; if the unit turns out
    // to have an earlier certificate, a bar on the worksheet says the new one
    // was not named as a revision. Storage failing or never answering (30 s)
    // gives an amber "not checked" bar. Also written to the error log.
    var timedOut = false, settled = false;
    var startedIso = new Date().toISOString();
    function note(tone, text) {
      try { if (global.LabCalLog && global.LabCalLog.notice) { global.LabCalLog.notice('certhistory', tone, text); return; } } catch (e) {}
      console.warn(text);
    }
    function notChecked() {
      note('amber', 'Earlier certificates for serial ' + (ctx.serial || '?') + ' were NOT checked (certificate storage did not answer).\n' +
        'If this unit already had a certificate on this job, the new one is not marked as revised (_rev2).');
    }
    var work = earlierCertificates(ctx, id.uid).then(function (prior) {
      settled = true;
      if (timedOut) {
        // ignore the certificate this very generation filed meanwhile
        prior = prior.filter(function (r) { return !r.savedAt || String(r.savedAt) < startedIso; });
        if (prior.length) {
          var s0 = prior.slice().sort(function (a, b) { return String(a.savedAt) < String(b.savedAt) ? -1 : 1; })[0];
          note('red', 'Serial ' + (ctx.serial || '?') + ' already had a certificate: ' +
            (s0.certRef || s0.summary || s0.filename) + ' \u2014 ' + dayLabel(s0.savedAt || s0.day) + '.\n' +
            'The check finished late, so the certificate just made was NOT named as a revision (_rev2). ' +
            'Tap Generate PDF again to make a correctly named revised certificate, and use that one.');
        } else {
          console.warn('Certificate history check finished late (serial ' + (ctx.serial || '?') + '): no earlier certificate.');
        }
      ...
      settled = true;
      console.warn('Certificate history check failed:', e);
      notChecked();
      return { ok: true, rev: 1 };
    });
    ...
    return Promise.race([work, timer]).then(function (r) {
      if (r === 'timeout') {
        timedOut = true;
        console.warn('Certificate history check still waiting after 2.5 s \u2014 carrying on, finishing it in the background.');
        setTimeout(function () { if (!settled) { settled = true; notChecked(); } }, (global.LabCalGuard && global.LabCalGuard.lateGiveUpMs) || 30000);
        r = { ok: true, rev: 1 };
      }
      r.uid = id.uid;
      if (r.ok) {
  ...
  global.LabCalGuard = {
    ...
    badChars: badChars,
    unprintableFields: unprintableFields,
